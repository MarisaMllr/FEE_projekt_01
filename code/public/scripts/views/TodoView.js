...
export default class TodoView {
  ...
  bindCreateTodo(handler) {
    document.querySelector('#btn-create-todo')
    .addEventListener('click', handler);
  }

  bindCloseDialog(handler) { this.#dialog.bindClose(handler); }
  bindSubmitForm(handler) { this.#dialog.bindSubmit(handler); }
  bindEditTodo(handler) { this.#list.bindEdit(handler); }
  openCreateDialog() { this.#dialog.openCreate(); }
